module ClassifiedsDecorations
  def listing
    h.content_tag :div do
      h.concat h.link_to object.title, "#{base_url}#{object.link}", :target => "_blank"
      h.concat h.tag("br")
      h.concat h.content_tag :span, object.author, :class => "author"
    end
  end
  ...
  def formatted_last_post
    ...
    FORMATS.each do |format|
      m = object.last_post.match(format)
